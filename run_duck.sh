...
APP_DIR="${DUCK_APP_DIR:-/home/duck/open_duck_mini_runtime_release_entry_clean}"
PYTHON="${DUCK_PYTHON:-/home/duck/.venv/bin/python}"
LOG_FILE="${DUCK_LOG_FILE:-/tmp/duck.log}"
PID_FILE="${DUCK_PID_FILE:-/tmp/duck_walk.pid}"
MODEL="${DUCK_ONNX_MODEL:-BEST_WALK_ONNX_2.onnx.duckenc}"
CONFIG="${DUCK_CONFIG:-$APP_DIR/duck_config.json}"
LICENSE="${DUCK_LICENSE:-$APP_DIR/license.duck}"
...
Environment overrides:
  ...
  DUCK_ONNX_MODEL=BEST_WALK_ONNX_2.onnx.duckenc DUCK_LICENSE=$APP_DIR/license.duck
...
ensure_app() {
    cd "$APP_DIR"
    export PYTHONPATH="$APP_DIR:$APP_DIR/mini_bdx_runtime:${PYTHONPATH:-}"
    if [ ! -x "$PYTHON" ]; then
    ...
    if [[ "$MODEL" == *.duckenc ]] && [ ! -f "$LICENSE" ]; then
...
runtime_pids() {
    pgrep -f "[s]cripts/v2_rl_walk_mujoco\.(py|bin)" || true
}
...
start_duck() {
    ...
    nohup scripts/v2_rl_walk_mujoco.bin \
...
start_duck_foreground() {
    ...
    echo "[START] min_motor_voltage=$MIN_MOTOR_VOLTAGE"
    exec scripts/v2_rl_walk_mujoco.bin \
        --onnx_model_path "$MODEL" \
        --license_path "$LICENSE" \
        --duck_config_path "$CONFIG" \
        --commands \
